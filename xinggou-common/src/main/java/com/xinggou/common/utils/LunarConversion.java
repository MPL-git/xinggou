...
public class LunarConversion {
    ...
    private static void checkLunarDate(int lunarYear, int lunarMonth, int lunarDay, boolean leapMonthFlag) {
        if ((lunarYear < MIN_YEAR) || (lunarYear > MAX_YEAR)) {
            throw new BizException("非法农历年份！");
        }
        if ((lunarMonth < 1) || (lunarMonth > 12)) {
            throw new BizException("非法农历月份！");
        }
        if ((lunarDay < 1) || (lunarDay > 30)) { // 中国的月最多30天
            throw new BizException("非法农历天数！");
        }

        int leap = getLeapMonth(lunarYear);// 计算该年应该闰哪个月
        if (leapMonthFlag && (lunarMonth != leap)) {
            throw new BizException("非法闰月！");
        }
    }
     ...
     * @return 阳历日期, 格式：yyyy-MM-dd HH:mm:ss
     */
    public static String lunarToSolar(String lunarDate, boolean leapMonthFlag) {
        ...
        if (leapMonthFlag & leapMonth != lunarMonth) {
            throw (new BizException("您输入的闰月标志有误！"));
        }
        ...
        if (leapMonth == 0 || (lunarMonth < leapMonth) || (lunarMonth == leapMonth && !leapMonthFlag)) {
            ...
            if (lunarDay > getMonthDays(lunarYear, lunarMonth)) {
                throw (new BizException("不合法的农历日期！"));
            }
    ...
    public static String solarToLunar(String solarDate, Boolean isFormat) {
        ...
        return "阴历:" + lunarYear + "年" + lunarMonth + "月" + lunarDay + "日";
    }
    ...
    public static String solarToLunar(String solarDate) {
        ...
        return StrKit.buildMsg("{}-{}-{}", lunarYear, fillWithZero(lunarMonth), fillWithZero(lunarDay));
    }
...
/*    public static void main(String[] args) throws Exception {
        System.out.println(LunarConversion.lunarToSolar("1990-12-04", false));
        System.out.println(LunarConversion.lunarToSolar("1984-10-21", true));
        System.out.println(LunarConversion.lunarToSolar("1990-12-25", false));
        System.out.println("************");
        System.out.println(LunarConversion.solarToLunar("1900-09-23", true));
        System.out.println(LunarConversion.solarToLunar("1900-09-24", true));
        System.out.println(LunarConversion.solarToLunar("1900-10-22", true));
        System.out.println(LunarConversion.solarToLunar("1900-10-23", true));
        System.out.println(LunarConversion.solarToLunar("1991-02-09", true));

        System.out.println(LunarConversion.solarToLunar("1990-06-30", true));
        System.out.println(LunarConversion.solarToLunar("1984-12-13", true));
        System.out.println(LunarConversion.solarToLunar("1991-01-19", true));
    }*/
